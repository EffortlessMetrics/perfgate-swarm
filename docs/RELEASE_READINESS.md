...
| No-panic governance | Advisory (v0.18.1 deferred) | `cargo +1.95.0 run -p xtask -- policy check-no-panic-family` is the active drift detector. On 2026-06-11 it reported `213 no-panic policy issue(s) found`. v0.18.1 treats this as a reviewed advisory debt condition, not a release-passing claim. |
