...
pub fn render_onboarding_readme(
    config_path: &Path,
    ci_path: Option<&Path>,
    has_benchmarks: bool,
) -> String {
    ...
    let ci_commit = ci_path
        ...
        .unwrap_or_default();
    let next_steps = if has_benchmarks {
        format!(
            r#"1. Run `perfgate check --config {config} --all`.
...
perfgate:
  stage: test
  before_script:
    - cargo install perfgate-cli --locked --version {version}
  script:
  ...
  pull-requests:
    '**':
      - step:
          ...
          script:
            - cargo install perfgate-cli --locked --version {version}
            - perfgate check --config {config_path} --all --mode cockpit --out-dir artifacts/perfgate || PERFGATE_EXIT=$?
  ...
  perfgate:
    ...
    steps:
      ...
      - run:
          ...
          command: cargo install perfgate-cli --locked --version {version}
      - run:
          name: Run benchmarks
          command: |
            perfgate check --config {config_path} --all --mode cockpit --out-dir artifacts/perfgate || EXIT=$?
            exit ${{EXIT:-0}}
      - store_artifacts:
...
"#,
    ...
    #[test]
    fn onboarding_readme_mentions_bench_entry_when_none_discovered() {
    ...
    fn scaffold_gitlab_ci() {
        ...
        assert!(content.contains("image: rust:1.95.0"));
        assert!(content.contains(&install));
        assert!(content.contains("when: always"));
    }
    ...
    fn scaffold_bitbucket_ci() {
        ...
        assert!(content.contains("image: rust:1.95.0"));
        assert!(content.contains(&install));
        assert!(content.contains("|| PERFGATE_EXIT=$?"));
        assert!(content.contains("exit ${PERFGATE_EXIT:-0}"));
    }
    ...
    fn scaffold_circleci_ci() {
        ...
        assert!(content.contains("cimg/rust:1.95.0"));
        assert!(content.contains(&install));
