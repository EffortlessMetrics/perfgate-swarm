...
2. Promote a trusted first run with `perfgate promote --current artifacts/perfgate/<bench>/run.json --to baselines/<bench>.json`.
...
"#,
    ...
    fn onboarding_readme_mentions_artifacts_and_next_steps() {
        ...
        assert!(content.contains("perfgate promote --current"));
    }
